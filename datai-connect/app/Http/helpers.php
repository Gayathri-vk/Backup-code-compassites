...
function popularUsers($popular)
{
    $suggested_users = '';
    if(count($popular) > 0){

        $popularuser = implode(",", $popular);
        $suggested_users = '';
        $suggested_users = App\User::whereIn('id', $popular)->where('id', '!=', Auth::user()->id)->orderBy(DB::raw("FIELD(id, $popularuser)"))->get();
    
        if (count($suggested_users) > 0) {
            /*if (count($suggested_users) > (int) Setting::get('min_items_page', 5)) {
                $suggested_users = $suggested_users->random((int) Setting::get('min_items_page', 5));
...
function suggestedUsers()
{
   /* $suggested_users = '';
    ...
    $suggested_users = '';
    $suggested_users = App\User::whereNotIn('id', Auth::user()->following()->get()->pluck('id'))->where('id', '!=', Auth::user()->id)->orderBy('id','desc')->take(5)->get();

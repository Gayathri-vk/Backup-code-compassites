...
class TimelineController extends AppBaseController
{
    ...
    public function showFeed(Request $request)
    {
        ...
        $followers_count = DB::table('followers')
                            ->join('users','users.id','followers.leader_id')
                            ->select(DB::raw('count(*) as follower_count'), 'followers.leader_id')
                            ->groupBy('leader_id')
                            ->where('followers.leader_id','!=',$id)
                            ->orderBy('follower_count','desc')
                            ->whereNotIn('users.id',Auth::user()->following()->get()->pluck('id'))
        ...
        {
            if($icount <=5)
            array_push($popular, $count->leader_id);
            else
            break;
            $icount++;
        }
        ...
        $popularUsers = popularUsers($popular);
        ...
        return $theme->scope('home', compact('timeline', 'posts', 'next_page_url', 'trending_tags', 'suggested_users', 'active_announcement', 'suggested_groups', 'suggested_pages', 'mode', 'user_post','followers_count', 'popularUsers'))
        ->render();
    }
    ...
    public function singlePost($post_id)
    {
        ...
        return $theme->scope('timeline/single-post', compact('post', 'timeline', 'suggested_users', 'trending_tags', 'suggested_groups', 'suggested_pages', 'mode','popularUsers'))->render();
    }
    ...
    public function eventsList(Request $request, $username)
    {
        ...
        return $theme->scope('home', compact('next_page_url', 'trending_tags', 'suggested_users', 'suggested_groups', 'suggested_pages', 'mode', 'user_events', 'username','popularUsers'))
        ->render();
    }
    ...
    public function addEvent($username, $group_id = null)
    {
        ...
        $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
        return $theme->scope('event-create', compact('suggested_users', 'suggested_groups', 'suggested_pages', 'username', 'group_id', 'timeline_name','popularUsers'))
            ->render();
    }
    ...
    public function allNotifications()
    {
        ...
        return $theme->scope('timeline/single-post', compact('notifications', 'suggested_users', 'trending_tags', 'suggested_groups', 'suggested_pages', 'mode','popularUsers'))->render();
    }
    ...
    public function createAlbum($username)
    {
        ...
        return $theme->scope('albums/create', compact('suggested_users', 'suggested_groups', 'suggested_pages', 'timeline','popularUsers'))->render();
    }

...
class AdminController extends Controller
{
    ...
    public function editRole($id)
    {
        ...
        $selectedPermission = $this->findByRoleId($id, true);
        return $theme->scope('admin/roles/edit', compact('permissions', 'userRole' , 'roles' , 'selectedPermission', 'id'))->render(); 
    }
    ...
    public function updateRole(Request $request)
    {
        ...
        $validatorObj = Validator::make($request->all(), [
            'rolename' => 'required|unique:roles,name,'.$id.',id',
        ]);
        if ($validatorObj->fails()) {
            return redirect()->back()
                  ->withInput($request->all())
                  ->withErrors($validatorObj->errors());
        }
       
        $roles = ['RoleName' => \Request::get('name'), 'DisplayName' => \Request::get('name'), 'Description' => \Request::get('name')];
        $permissions = \Input::get('permissions');
        $result = $this->rolePermissionRepo->updatePermissionsForRole($permissions, $id, $roles);
            if($result)
            {
    ...
    public function updatePermissionsForRole($permissions, $roleId, $roles)
    {
        // Fetch Role
        $role = Role::find($roleId);
        $role->name = $roles['RoleName'];
        $role->display_name = $roles['DisplayName'];
        ...
        if($role->save())
        {
            ...
            $this->saveRolePermission($role->id, $permissions);
            return $role;
        }
        return false;
    }
    ...
    {
        $roleData = $this->findRole($roleId);
            foreach ($permissions as $per) {
                $permission = $this->findPermission($per);
                $roleData->attachPermission($permission);
            }
    ...
    public function deleteRolePermissionsByRoleId($RoleId)
    {
        ...
        return RolesPermission::where('role_id', $RoleId)->delete();
    }

    public function findByRoleId($roleId, $responseArray = false)
    {
       $rolePermissions = RolesPermission::where('role_id',$roleId)
            ->lists('permission_id');
    ...
    public function storeRole(Request $request)
    {
        ...
        $roles = ['RoleName' => \Request::get('rolename'), 'DisplayName' => \Request::get('rolename'), 'Description' => \Request::get('rolename')];
        $permissions = \Input::get('permissions');
        $role = new Role();
        $role->name = $roles['RoleName'];
        $role->display_name = $roles['DisplayName'];
    ...
    public function createRole()
    {
       $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('admin');
        $theme->setTitle(trans('common.general_settings').' '.Setting::get('title_seperator').' '.Setting::get('site_title').' '.Setting::get('title_seperator').' '.Setting::get('site_tagline'));
 $permissions = Permission::all();
        return $theme->scope('admin/roles/create', compact('permissions'))->render(); 
    }
